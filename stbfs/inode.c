...
static int stbfs_unlink(struct inode *dir, struct dentry *dentry){
	...
	struct path lower_path;
	char *new_file = NULL, *old_path = NULL; 
	char *path_buf = NULL, *root_path = NULL; 
	struct timespec time;
    struct rtc_time tm;
	struct super_block *sb = NULL;
	struct stbfs_sb_info *sbinfo = NULL;
	uid_t user_id = current_uid().val;

	printk("Inside stbfs unlink\n");
	stbfs_get_lower_path(dentry, &lower_path);
	lower_dentry = lower_path.dentry;
	dget(lower_dentry);
	...
	sb = dentry->d_sb;
	sbinfo = STBFS_SB(sb);
	root_path = sbinfo->root_path;  
	printk("ROOT PATH in inode : %s\n", root_path);
	...
	printk("OLD PATH : %s\n", old_path);
    if(is_file_in_stb(lower_dir_dentry)){
        printk("This file is within .stb\n");
        goto unlink;
    }
    ...
    if (enc_key!=NULL && strlen(enc_key)!=0){
		sprintf(new_file,"%s/%s/%04d-%02d-%02d-%02d-%02d-%02d-%s-%d%s",\
			root_path, ".stb", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec,\
			lower_dentry->d_name.name, user_id,".enc");
		printk("Encrypting file: %s with enc_key %s\n", new_file, enc_key);
        err = file_read_write_helper(old_path, new_file, 1);
    }
    else{
		sprintf(new_file,"%s/%s/%04d-%02d-%02d-%02d-%02d-%02d-%s-%d",\
			root_path, ".stb", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec,\
			lower_dentry->d_name.name, user_id);
		printk("Unlinking file %s without encryption\n", new_file);
		err = file_read_write_helper(old_path, new_file, 0);
	...
	unlink:
	// #ifdef EXTRA_CREDIT
	// 	printk("clearing file bytes before unlinking\n");
	// 	err = clear_file_bytes(old_path);
	// 	if (err)
	// 		goto out; 
	// #endif
			 ...
			 unsigned int flags)
{
	...
	printk("Implementing rename from %s to %s\n", lower_old_dentry->d_name.name, lower_new_dentry->d_name.name);
	printk("old dir %s new dir %s\n", lower_old_dir_dentry->d_name.name, lower_new_dir_dentry->d_name.name);

	if(is_file_in_stb(lower_old_dir_dentry) || is_file_in_stb(lower_new_dir_dentry)){
		printk("Files inside .stb cannot be renamed\n");
		err = -EPERM;
		goto out;
	}
...
static int stbfs_setattr(struct dentry *dentry, struct iattr *ia)
{
	...
	printk("filename : %s parent dir : %s\n", lower_dentry->d_name.name, lower_dir_dentry->d_name.name);
	if (is_file_in_stb(lower_dir_dentry)){
		err = -EPERM;
		printk("File permission change within .stb is forbidden\n");
		goto out;
	}
	else 
		printk("Changing file permission\n");
